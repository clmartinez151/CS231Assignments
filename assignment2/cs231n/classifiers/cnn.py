...
class ThreeLayerConvNet(object):
    ...
    def __init__(self, input_dim=(3, 32, 32), num_filters=32, filter_size=7,
        ...
        Hprime = 1 + (H + 2*pad - filter_size)//stride
        Wprime = 1 + (W + 2*pad - filter_size)//stride
        
        #pool height, width, and stride = 2
        Hprime_pool = 1 + (Hprime - 2)//2 
        Wprime_pool = 1 + (Hprime - 2)//2
        
        self.params['W1'] = weight_scale * np.random.randn(num_filters, 3, filter_size, filter_size)
        self.params['b1'] = np.zeros(num_filters)
        self.params['W2'] = weight_scale * np.random.randn(num_filters*Hprime_pool*Wprime_pool, hidden_dim)
